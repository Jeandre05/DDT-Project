...
class User:
 ...
 def save_to_file(self):
      with open("user_data.txt", "a") as file:
         file.write(f"{self.email},{self.password},{self.name},{self.dob},{self.pr_squat},{self.pr_bench},{self.pr_deadlift}\n")
...
def change_details():
   ...
   user = User(email, password, name, dob, pr_squat, pr_bench, pr_deadlift)
   user.save_to_file()
...
#Create a varibale to hold choice in
preference_choice = StringVar()

#Create the different choices of the radio button
radio_button_strength = ttk.Radiobutton(importance_frame, text="Want to improve my overall strentgh in the gym!", variable=preference_choice, value="Option 1")
radio_button_strength.grid(row = 0, column=3, padx=10 ,pady = 10, sticky="NW")

radio_button_recovery = ttk.Radiobutton(importance_frame, text="Want to recover from a recent injury!", variable=preference_choice, value="Option 2")
radio_button_recovery.grid(row = 1, column=3, padx=10 ,pady = 10, sticky="NW")

radio_button_endurance = ttk.Radiobutton(importance_frame, text="Want to imporve my muscular endurance through longer sets!", variable=preference_choice, value="Option 3")
radio_button_endurance.grid(row = 2, column=3, padx=10 ,pady = 10, sticky="NW")

#submit button
change_button = ttk.Button(root,text="Change",command=change_details)
...
#Open image
motivation_pic = Image.open("Motivation image Data.png")

#Resize image to 
resized = motivation_pic.resize((240,160), Image.ANTIALIAS)
motivation_final_pic = ImageTk.PhotoImage(resized)

#Paste image
motivation_image_label = Label(root, image=motivation_final_pic)
motivation_image_label.grid(row=4,column=1, padx=10)
